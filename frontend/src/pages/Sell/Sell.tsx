...
const ProductForm: React.FC = () => {
  ...
  if (id) {
    return <EditProduct id={id} categories={categories}/>
  }

  return <NewProduct categoris={categories} />;
};
function EditProduct({id,categories}:{id:string,categories:Category[] | null}){
    const {product, messageElement} = useProduct(id);
    if(!product){
        return messageElement;
    }
...
function NewProduct({ categoris }: { categoris?: Category[] | null }) {
  ...
  const handleSubmit = () => {
    setIsLoading(true);
    api
      .post("/sellers/products", productData, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then((response) => {
  ...
  if (createdProductId) {
    return (
      <div className="product-created-successfully">
        ...
        <Link
          to={`/seller-portal/products/${createdProductId}`}
          className="button"
        >
          عرض المنتج
        </Link>
...
function Form({
  ...
  return (
    <form className="sell-form" action="" ref={formRef}>
      ...
      />
      <InputError
        input={descriptionInputRef.current}
        name="وصف المنتج"
        triggerValidate={triggerValidate}
        detailedLengthError
      />
      ...
      <FileDropArea file={productData.image} setFile={setImageFile} />
      <FileInputError
        input={fileInputRef.current}
        file={productData.image}
        allowedTypes={allowedFileTypes}
        maxSizeInBytes={2048 * 1024}
        required={imageRequeried}
        triggerValidate={triggerValidate}
      />
      <button
        type="submit"
        className="submittionbutton"
        onClick={submit}
        disabled={isLoading}
      >
