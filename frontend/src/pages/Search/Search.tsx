...
async function load(query: string, abortController: AbortController | null,
    ...
    try {
        ...
        if (isAxiosError(error) && error.request && !error.response) {
             setErrorElement(<div className="error-message">تعذر الاتصال, تحقق من الاتصال بالشبكة.{retryButton()}</div>);
        }
        else {
            setErrorElement(<div className="error-message">حدث خطأ ما.{retryButton()}</div>);
        }
    }
    finally {
...
function useSearchResult(): [products:Product[],JSX.Element | null,() => boolean] {
    ...
    const RetryButton = (retry: () => Promise<void>) => <button className="retry-button" onClick={retry}>أعد المحاولة.</button>;
    const loadSearchResult = async () => { await load(query, abortController, setAbortController, setErrorElement, () => RetryButton(loadSearchResult), setProducts) };
    const isLoading = () => abortController != null;
    //load search result everytime location changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    useEffect(() => { loadSearchResult() }, [location]);
